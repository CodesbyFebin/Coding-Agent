...
export const codingagentPythonDataScience: PillarEditorial = {
  pillarId: 'codingagent-python-data-science',
  updated: '2026-09-16',
  definition: 'Specialized agentic workflows for Python development, automated Jupyter notebook analysis, Pandas pipeline refactoring, and data science.',
  sections: [
    {
      heading: 'What CodingAgent Python & Data Science Actually Does',
      paragraphs: [
        'Specialized agentic workflows for Python development, automated Jupyter notebook analysis, Pandas pipeline refactoring, and data science. Within CodingAgent.in\'s broader agentic engineering platform, this pillar is not a standalone feature toggle but a design constraint that shapes how the surrounding India & Solutions components are allowed to behave. Every capability described here is scoped by the same governance model the rest of the platform uses: an explicit boundary between what a model may reason about and what a tool is actually permitted to execute.',
        'Optimizes agent execution for virtual environments, NumPy vectorization, and data pipeline verification. That is the practical justification for treating this as its own architectural pillar rather than folding it into a more general capability: the failure modes it addresses are specific enough that a generic policy would either under-protect or over-restrict the surrounding workflow.',
      ],
      bullets: [
        'Tag: python',
        'Tag: data-science',
    ...
    },
    {
      ...
      paragraphs: [
        'Teams adopting \'CodingAgent Python & Data Science\' should start by confirming the boundary described above actually matches their own risk tolerance -- the default configuration reflects a reasonable general-purpose posture, not necessarily the most restrictive (or most permissive) one available. Where the platform exposes configuration for this pillar, treat it the same way you would treat any other security- or correctness-relevant configuration: version it, review changes to it, and test that a change actually has the effect you expect before relying on it in a live workflow.',
    ...
    },
    {
      ...
      paragraphs: [
        'Rather than taking any architectural description at face value -- including this one -- the more useful exercise for a team evaluating CodingAgent.in is to write down the specific failure scenario \'CodingAgent Python & Data Science\' claims to prevent, and then check whether the platform\'s actual verification mechanism (described above) would catch that exact scenario if it happened. If it would not, that\'s a real gap worth raising, not a reason to distrust the pillar model in general -- the whole premise of naming these things explicitly is so gaps are locatable and fixable rather than hidden inside a vague, unauditable system prompt.',
        'The href for this page (`/python-data-science`) is a stable, canonical identifier once the pillar crosses the platform\'s own indexability bar -- so it\'s reasonable to bookmark or cite directly when tracking an evaluation decision back to the specific architectural claim that informed it.',
      ],
    },
  ],
  faq: [
    {
      question: 'What problem does CodingAgent Python & Data Science actually solve?',
      answer: 'Specialized agentic workflows for Python development, automated Jupyter notebook analysis, Pandas pipeline refactoring, and data science. Optimizes agent execution for virtual environments, NumPy vectorization, and data pipeline verification.',
    },
    {
      question: 'How is completion or correctness verified for this pillar?',
      answer: 'Automated pytest suite execution and data schema validation in sandboxed environments.',
    },
    {
    ...
    },
    {
      ...
      answer: 'A misconfiguration in the \'CodingAgent Python & Data Science\' area is designed to fail toward the more restrictive behavior rather than silently degrading to a more permissive one -- consistent with the platform\'s general ALLOW/ASK/DENY posture, an unclear or failed check defaults to requiring explicit human approval rather than proceeding automatically.',
    },
    {
      question: 'How does this pillar relate to CodingAgent Build Verification, CodingAgent Unit Test Verification?',
      answer: 'It composes directly with CodingAgent Build Verification, CodingAgent Unit Test Verification, CodingAgent Agent Sandboxing: none of these are meant to be adopted in isolation, and the platform\'s guarantees in this area assume the related pillars are also in place around it.',
    },
    {
    ...
    },
    {
      ...
      answer: 'It is categorized under India & Solutions, tagged python, data-science, pandas, jupyter.',
    },
    {
      question: 'Who should read this page before adopting CodingAgent Python & Data Science?',
    ...
    {
      question: 'What\'s the recommended rollout sequence for CodingAgent Python & Data Science?',
