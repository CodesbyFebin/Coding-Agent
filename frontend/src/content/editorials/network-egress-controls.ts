...
export const networkEgressControls: PillarEditorial = {
  ...
  sections: [
    ...
    },
    {
      ...
      paragraphs: [
        'Teams adopting \'CodingAgent Network Egress Controls\' should start by confirming the boundary described above actually matches their own risk tolerance -- the default configuration reflects a reasonable general-purpose posture, not necessarily the most restrictive (or most permissive) one available. Where the platform exposes configuration for this pillar, treat it the same way you would treat any other security- or correctness-relevant configuration: version it, review changes to it, and test that a change actually has the effect you expect before relying on it in a live workflow.',
    ...
    },
    {
      ...
      paragraphs: [
        'Rather than taking any architectural description at face value -- including this one -- the more useful exercise for a team evaluating CodingAgent.in is to write down the specific failure scenario \'CodingAgent Network Egress Controls\' claims to prevent, and then check whether the platform\'s actual verification mechanism (described above) would catch that exact scenario if it happened. If it would not, that\'s a real gap worth raising, not a reason to distrust the pillar model in general -- the whole premise of naming these things explicitly is so gaps are locatable and fixable rather than hidden inside a vague, unauditable system prompt.',
        'The href for this page (`/network-egress-controls`) is a stable, canonical identifier once the pillar crosses the platform\'s own indexability bar -- so it\'s reasonable to bookmark or cite directly when tracking an evaluation decision back to the specific architectural claim that informed it.',
      ],
    },
  ],
  faq: [
    {
      question: 'What problem does CodingAgent Network Egress Controls actually solve?',
      answer: 'Default-deny network firewall allowing agent tool requests only to approved, domain-allowlisted endpoints. Stops data exfiltration attacks where compromised code attempts to curl environment variables to remote adversary servers.',
    },
    {
      question: 'How is completion or correctness verified for this pillar?',
      answer: 'Packet inspection logging and DNS filtering on all container egress interfaces.',
    },
    {
    ...
    },
    {
      ...
      answer: 'It is categorized under Security & Sovereignty, tagged firewall, egress, data-loss-prevention.',
    },
    {
      question: 'Who should read this page before adopting CodingAgent Network Egress Controls?',
    ...
    {
      question: 'What\'s the recommended rollout sequence for CodingAgent Network Egress Controls?',
