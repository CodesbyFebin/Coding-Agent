...
export const ideIntegrations: PillarEditorial = {
  ...
  updated: '2026-09-16',
  definition: 'Native extensions for VS Code, JetBrains IDEs, and Neovim providing inline diffs, gutter approvals, and chat panels.',
  sections: [
    {
      heading: 'What CodingAgent IDE Integrations Actually Does',
      paragraphs: [
        'Native extensions for VS Code, JetBrains IDEs, and Neovim providing inline diffs, gutter approvals, and chat panels. Within CodingAgent.in\'s broader agentic engineering platform, this pillar is not a standalone feature toggle but a design constraint that shapes how the surrounding Enterprise & Tooling components are allowed to behave. Every capability described here is scoped by the same governance model the rest of the platform uses: an explicit boundary between what a model may reason about and what a tool is actually permitted to execute.',
        'Brings sovereign agent capabilities directly into the editor where developers spend their working hours. That is the practical justification for treating this as its own architectural pillar rather than folding it into a more general capability: the failure modes it addresses are specific enough that a generic policy would either under-protect or over-restrict the surrounding workflow.',
      ],
      bullets: [
        'Tag: ide',
        'Tag: vscode',
    ...
    },
    {
      ...
      paragraphs: [
        'Teams adopting \'CodingAgent IDE Integrations\' should start by confirming the boundary described above actually matches their own risk tolerance -- the default configuration reflects a reasonable general-purpose posture, not necessarily the most restrictive (or most permissive) one available. Where the platform exposes configuration for this pillar, treat it the same way you would treat any other security- or correctness-relevant configuration: version it, review changes to it, and test that a change actually has the effect you expect before relying on it in a live workflow.',
    ...
    },
    {
      ...
      paragraphs: [
        'Rather than taking any architectural description at face value -- including this one -- the more useful exercise for a team evaluating CodingAgent.in is to write down the specific failure scenario \'CodingAgent IDE Integrations\' claims to prevent, and then check whether the platform\'s actual verification mechanism (described above) would catch that exact scenario if it happened. If it would not, that\'s a real gap worth raising, not a reason to distrust the pillar model in general -- the whole premise of naming these things explicitly is so gaps are locatable and fixable rather than hidden inside a vague, unauditable system prompt.',
        'The href for this page (`/ide-integrations`) is a stable, canonical identifier once the pillar crosses the platform\'s own indexability bar -- so it\'s reasonable to bookmark or cite directly when tracking an evaluation decision back to the specific architectural claim that informed it.',
      ],
    },
  ],
  faq: [
    {
      question: 'What problem does CodingAgent IDE Integrations actually solve?',
      answer: 'Native extensions for VS Code, JetBrains IDEs, and Neovim providing inline diffs, gutter approvals, and chat panels. Brings sovereign agent capabilities directly into the editor where developers spend their working hours.',
    },
    {
      question: 'How is completion or correctness verified for this pillar?',
      answer: 'LSP (Language Server Protocol) compliance for code navigation and symbol lookup.',
    },
    {
    ...
    },
    {
      ...
      answer: 'It is categorized under Enterprise & Tooling, tagged ide, vscode, jetbrains.',
    },
    {
      question: 'Who should read this page before adopting CodingAgent IDE Integrations?',
    ...
    {
      question: 'What\'s the recommended rollout sequence for CodingAgent IDE Integrations?',
