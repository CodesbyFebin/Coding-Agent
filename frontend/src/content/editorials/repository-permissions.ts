...
export const repositoryPermissions: PillarEditorial = {
  ...
  sections: [
    ...
    },
    {
      ...
      paragraphs: [
        'Rather than taking any architectural description at face value -- including this one -- the more useful exercise for a team evaluating CodingAgent.in is to write down the specific failure scenario \'CodingAgent Repository Permissions\' claims to prevent, and then check whether the platform\'s actual verification mechanism (described above) would catch that exact scenario if it happened. If it would not, that\'s a real gap worth raising, not a reason to distrust the pillar model in general -- the whole premise of naming these things explicitly is so gaps are locatable and fixable rather than hidden inside a vague, unauditable system prompt.',
        'The href for this page (`/repository-permissions`) is a stable, canonical identifier once the pillar crosses the platform\'s own indexability bar -- so it\'s reasonable to bookmark or cite directly when tracking an evaluation decision back to the specific architectural claim that informed it.',
      ],
    },
  ],
  faq: [
    {
      question: 'What problem does CodingAgent Repository Permissions actually solve?',
      answer: 'Granular file and branch access policies restricting which folders an agent can read, modify, or delete. Prevents unintentional changes to mission-critical infrastructure-as-code files, root manifests, or branch protections.',
    },
    {
      question: 'How is completion or correctness verified for this pillar?',
      answer: 'Pre-commit git hooks validating diffs against declared task scope manifests.',
    },
    {
    ...
    },
    {
      ...
      answer: 'It is categorized under Security & Sovereignty, tagged git, permissions, branch-protection.',
    },
    {
      question: 'Who should read this page before adopting CodingAgent Repository Permissions?',
    ...
    {
      question: 'What\'s the recommended rollout sequence for CodingAgent Repository Permissions?',
