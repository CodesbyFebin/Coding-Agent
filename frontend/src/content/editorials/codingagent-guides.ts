...
export const codingagentGuides: PillarEditorial = {
  ...
  sections: [
    ...
    },
    {
      ...
      paragraphs: [
        'Teams adopting \'CodingAgent Guides\' should start by confirming the boundary described above actually matches their own risk tolerance -- the default configuration reflects a reasonable general-purpose posture, not necessarily the most restrictive (or most permissive) one available. Where the platform exposes configuration for this pillar, treat it the same way you would treat any other security- or correctness-relevant configuration: version it, review changes to it, and test that a change actually has the effect you expect before relying on it in a live workflow.',
    ...
    },
    {
      ...
      paragraphs: [
        'Rather than taking any architectural description at face value -- including this one -- the more useful exercise for a team evaluating CodingAgent.in is to write down the specific failure scenario \'CodingAgent Guides\' claims to prevent, and then check whether the platform\'s actual verification mechanism (described above) would catch that exact scenario if it happened. If it would not, that\'s a real gap worth raising, not a reason to distrust the pillar model in general -- the whole premise of naming these things explicitly is so gaps are locatable and fixable rather than hidden inside a vague, unauditable system prompt.',
        'The href for this page (`/guides`) is a stable, canonical identifier once the pillar crosses the platform\'s own indexability bar -- so it\'s reasonable to bookmark or cite directly when tracking an evaluation decision back to the specific architectural claim that informed it.',
      ],
    },
  ],
  faq: [
    {
      question: 'What problem does CodingAgent Guides actually solve?',
      answer: 'Step-by-step tutorials for configuring local Ollama models, writing custom MCP tools, and setting up air-gapped runtimes. Walks practitioners through end-to-end setups tailored to specific developer environments.',
    },
    {
      question: 'How is completion or correctness verified for this pillar?',
      answer: 'Interactive configuration templates validated against latest runtime releases.',
    },
    {
    ...
    },
    {
      ...
      answer: 'It is categorized under Platform Pillars, tagged tutorials, recipes, local-setup.',
    },
    {
      question: 'Who should read this page before adopting CodingAgent Guides?',
    ...
    {
      question: 'What\'s the recommended rollout sequence for CodingAgent Guides?',
    ...
    },
    {
      ...
      answer: 'Just this one, deliberately. \'CodingAgent Guides\' does not silently absorb responsibility for adjacent concerns like general model routing, workspace lifecycle, or organization-wide policy -- those are each their own pillars with their own explicit boundary. If a capability you need sounds adjacent but isn\'t covered here, check the knowledge graph\'s category grouping for the more precise pillar.',
    },
    {
      question: 'What is the canonical URL for this pillar once it\'s fully documented?',
      answer: '`/guides` on codingagent.in -- once an editorial crosses the platform\'s own indexability bar (currently 2,000 words of substantive, non-duplicated content), that URL becomes the canonical, sitemap-listed identifier for this pillar, suitable for bookmarking or citing directly in an evaluation writeup.',
    },
    {
      question: 'How does CodingAgent Guides fail -- does it fail open or fail closed?',
