...
export const workingMissionMemory: PillarEditorial = {
  pillarId: 'working-mission-memory',
  updated: '2026-09-16',
  definition: 'Structured state stores tracking active task objectives, hypotheses tested, failed approaches, and remaining TODOs.',
  sections: [
    {
      heading: 'What CodingAgent Working & Mission Memory Actually Does',
      paragraphs: [
        'Structured state stores tracking active task objectives, hypotheses tested, failed approaches, and remaining TODOs. Within CodingAgent.in\'s broader agentic engineering platform, this pillar is not a standalone feature toggle but a design constraint that shapes how the surrounding Verification & Memory components are allowed to behave. Every capability described here is scoped by the same governance model the rest of the platform uses: an explicit boundary between what a model may reason about and what a tool is actually permitted to execute.',
        'Prevents the agent from repeating previously failed edits and maintains momentum across multi-turn interactions. That is the practical justification for treating this as its own architectural pillar rather than folding it into a more general capability: the failure modes it addresses are specific enough that a generic policy would either under-protect or over-restrict the surrounding workflow.',
      ],
      bullets: [
        'Tag: mission-state',
        'Tag: hypotheses',
    ...
    },
    {
      ...
      paragraphs: [
        'Teams adopting \'CodingAgent Working & Mission Memory\' should start by confirming the boundary described above actually matches their own risk tolerance -- the default configuration reflects a reasonable general-purpose posture, not necessarily the most restrictive (or most permissive) one available. Where the platform exposes configuration for this pillar, treat it the same way you would treat any other security- or correctness-relevant configuration: version it, review changes to it, and test that a change actually has the effect you expect before relying on it in a live workflow.',
    ...
    },
    {
      ...
      paragraphs: [
        'Rather than taking any architectural description at face value -- including this one -- the more useful exercise for a team evaluating CodingAgent.in is to write down the specific failure scenario \'CodingAgent Working & Mission Memory\' claims to prevent, and then check whether the platform\'s actual verification mechanism (described above) would catch that exact scenario if it happened. If it would not, that\'s a real gap worth raising, not a reason to distrust the pillar model in general -- the whole premise of naming these things explicitly is so gaps are locatable and fixable rather than hidden inside a vague, unauditable system prompt.',
        'The href for this page (`/working-mission-memory`) is a stable, canonical identifier once the pillar crosses the platform\'s own indexability bar -- so it\'s reasonable to bookmark or cite directly when tracking an evaluation decision back to the specific architectural claim that informed it.',
      ],
    },
  ],
  faq: [
    {
      question: 'What problem does CodingAgent Working & Mission Memory actually solve?',
      answer: 'Structured state stores tracking active task objectives, hypotheses tested, failed approaches, and remaining TODOs. Prevents the agent from repeating previously failed edits and maintains momentum across multi-turn interactions.',
    },
    {
      question: 'How is completion or correctness verified for this pillar?',
      answer: 'Monotonic convergence metrics toward accepted criteria.',
    },
    {
    ...
    },
    {
      ...
      answer: 'It is categorized under Verification & Memory, tagged mission-state, hypotheses, scratchpad.',
    },
    {
      question: 'Who should read this page before adopting CodingAgent Working & Mission Memory?',
    ...
    {
      question: 'What\'s the recommended rollout sequence for CodingAgent Working & Mission Memory?',
