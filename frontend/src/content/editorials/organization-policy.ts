...
export const organizationPolicy: PillarEditorial = {
  ...
  updated: '2026-09-16',
  definition: 'Centrally managed, policy-as-code guardrails defining permitted models, external network boundaries, and required approval workflows.',
  sections: [
    {
      heading: 'What CodingAgent Organization Policy Actually Does',
      paragraphs: [
        'Centrally managed, policy-as-code guardrails defining permitted models, external network boundaries, and required approval workflows. Within CodingAgent.in\'s broader agentic engineering platform, this pillar is not a standalone feature toggle but a design constraint that shapes how the surrounding Enterprise & Tooling components are allowed to behave. Every capability described here is scoped by the same governance model the rest of the platform uses: an explicit boundary between what a model may reason about and what a tool is actually permitted to execute.',
        'Ensures individual developer agents adhere strictly to corporate data governance and legal compliance mandates. That is the practical justification for treating this as its own architectural pillar rather than folding it into a more general capability: the failure modes it addresses are specific enough that a generic policy would either under-protect or over-restrict the surrounding workflow.',
      ],
      bullets: [
        'Tag: opa',
        'Tag: policy-as-code',
    ...
    },
    {
      ...
      paragraphs: [
        'Rather than taking any architectural description at face value -- including this one -- the more useful exercise for a team evaluating CodingAgent.in is to write down the specific failure scenario \'CodingAgent Organization Policy\' claims to prevent, and then check whether the platform\'s actual verification mechanism (described above) would catch that exact scenario if it happened. If it would not, that\'s a real gap worth raising, not a reason to distrust the pillar model in general -- the whole premise of naming these things explicitly is so gaps are locatable and fixable rather than hidden inside a vague, unauditable system prompt.',
        'The href for this page (`/organization-policy`) is a stable, canonical identifier once the pillar crosses the platform\'s own indexability bar -- so it\'s reasonable to bookmark or cite directly when tracking an evaluation decision back to the specific architectural claim that informed it.',
      ],
    },
  ],
  faq: [
    {
      question: 'What problem does CodingAgent Organization Policy actually solve?',
      answer: 'Centrally managed, policy-as-code guardrails defining permitted models, external network boundaries, and required approval workflows. Ensures individual developer agents adhere strictly to corporate data governance and legal compliance mandates.',
    },
    {
      question: 'How is completion or correctness verified for this pillar?',
      answer: 'Open Policy Agent (OPA) / Rego policy engine validating agent configuration on boot.',
    },
    {
    ...
    },
    {
      ...
      answer: 'It is categorized under Enterprise & Tooling, tagged opa, policy-as-code, governance.',
    },
    {
      question: 'Who should read this page before adopting CodingAgent Organization Policy?',
    ...
    {
      question: 'What\'s the recommended rollout sequence for CodingAgent Organization Policy?',
    ...
    {
      question: 'How does CodingAgent Organization Policy fail -- does it fail open or fail closed?',
