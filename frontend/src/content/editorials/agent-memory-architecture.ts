...
export const agentMemoryArchitecture: PillarEditorial = {
  ...
  sections: [
    ...
    },
    {
      ...
      paragraphs: [
        'Teams adopting \'CodingAgent Agent Memory Architecture\' should start by confirming the boundary described above actually matches their own risk tolerance -- the default configuration reflects a reasonable general-purpose posture, not necessarily the most restrictive (or most permissive) one available. Where the platform exposes configuration for this pillar, treat it the same way you would treat any other security- or correctness-relevant configuration: version it, review changes to it, and test that a change actually has the effect you expect before relying on it in a live workflow.',
    ...
    },
    {
      ...
      paragraphs: [
        'Rather than taking any architectural description at face value -- including this one -- the more useful exercise for a team evaluating CodingAgent.in is to write down the specific failure scenario \'CodingAgent Agent Memory Architecture\' claims to prevent, and then check whether the platform\'s actual verification mechanism (described above) would catch that exact scenario if it happened. If it would not, that\'s a real gap worth raising, not a reason to distrust the pillar model in general -- the whole premise of naming these things explicitly is so gaps are locatable and fixable rather than hidden inside a vague, unauditable system prompt.',
        'The href for this page (`/agent-memory-architecture`) is a stable, canonical identifier once the pillar crosses the platform\'s own indexability bar -- so it\'s reasonable to bookmark or cite directly when tracking an evaluation decision back to the specific architectural claim that informed it.',
      ],
    },
  ],
  faq: [
    {
      question: 'What problem does CodingAgent Agent Memory Architecture actually solve?',
      answer: 'Hierarchical memory system separating short-term scratchpad context, working task memory, and long-term repository knowledge. Prevents context window overflow while preserving critical architectural lessons learned in prior debugging sessions.',
    },
    {
      question: 'How is completion or correctness verified for this pillar?',
      answer: 'Memory retrieval relevance evaluations and garbage collection of stale ephemeral records.',
    },
    {
    ...
    },
    {
      ...
      answer: 'It is categorized under Verification & Memory, tagged memory, hierarchical, context.',
    },
    {
      question: 'Who should read this page before adopting CodingAgent Agent Memory Architecture?',
    ...
    {
      question: 'What\'s the recommended rollout sequence for CodingAgent Agent Memory Architecture?',
