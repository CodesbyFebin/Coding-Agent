...
export const durableRuntimes: PillarEditorial = {
  ...
  updated: '2026-09-16',
  definition: 'Resilient execution runtimes capable of pausing, serializing, persisting to disk, and resuming missions across process restarts.',
  sections: [
    {
      heading: 'What CodingAgent Durable Runtimes Actually Does',
      paragraphs: [
        'Resilient execution runtimes capable of pausing, serializing, persisting to disk, and resuming missions across process restarts. Within CodingAgent.in\'s broader agentic engineering platform, this pillar is not a standalone feature toggle but a design constraint that shapes how the surrounding Core & Agents components are allowed to behave. Every capability described here is scoped by the same governance model the rest of the platform uses: an explicit boundary between what a model may reason about and what a tool is actually permitted to execute.',
        'Allows long-running multi-hour migrations or flaky network interruptions to resume seamlessly without lost context. That is the practical justification for treating this as its own architectural pillar rather than folding it into a more general capability: the failure modes it addresses are specific enough that a generic policy would either under-protect or over-restrict the surrounding workflow.',
      ],
      bullets: [
        'Tag: persistence',
        'Tag: checkpointing',
    ...
    },
    {
      ...
      paragraphs: [
        'Teams adopting \'CodingAgent Durable Runtimes\' should start by confirming the boundary described above actually matches their own risk tolerance -- the default configuration reflects a reasonable general-purpose posture, not necessarily the most restrictive (or most permissive) one available. Where the platform exposes configuration for this pillar, treat it the same way you would treat any other security- or correctness-relevant configuration: version it, review changes to it, and test that a change actually has the effect you expect before relying on it in a live workflow.',
    ...
    },
    {
      ...
      paragraphs: [
        'Rather than taking any architectural description at face value -- including this one -- the more useful exercise for a team evaluating CodingAgent.in is to write down the specific failure scenario \'CodingAgent Durable Runtimes\' claims to prevent, and then check whether the platform\'s actual verification mechanism (described above) would catch that exact scenario if it happened. If it would not, that\'s a real gap worth raising, not a reason to distrust the pillar model in general -- the whole premise of naming these things explicitly is so gaps are locatable and fixable rather than hidden inside a vague, unauditable system prompt.',
        'The href for this page (`/durable-runtimes`) is a stable, canonical identifier once the pillar crosses the platform\'s own indexability bar -- so it\'s reasonable to bookmark or cite directly when tracking an evaluation decision back to the specific architectural claim that informed it.',
      ],
    },
  ],
  faq: [
    {
      question: 'What problem does CodingAgent Durable Runtimes actually solve?',
      answer: 'Resilient execution runtimes capable of pausing, serializing, persisting to disk, and resuming missions across process restarts. Allows long-running multi-hour migrations or flaky network interruptions to resume seamlessly without lost context.',
    },
    {
      question: 'How is completion or correctness verified for this pillar?',
      answer: 'Snapshot replay tests verifying that restored states reproduce deterministic downstream operations.',
    },
    {
    ...
    },
    {
      ...
      answer: 'It is categorized under Core & Agents, tagged persistence, checkpointing, durability.',
    },
    {
      question: 'Who should read this page before adopting CodingAgent Durable Runtimes?',
    ...
    {
      question: 'What\'s the recommended rollout sequence for CodingAgent Durable Runtimes?',
