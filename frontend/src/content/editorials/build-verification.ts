...
export const buildVerification: PillarEditorial = {
  ...
  sections: [
    ...
    },
    {
      ...
      paragraphs: [
        'Rather than taking any architectural description at face value -- including this one -- the more useful exercise for a team evaluating CodingAgent.in is to write down the specific failure scenario \'CodingAgent Build Verification\' claims to prevent, and then check whether the platform\'s actual verification mechanism (described above) would catch that exact scenario if it happened. If it would not, that\'s a real gap worth raising, not a reason to distrust the pillar model in general -- the whole premise of naming these things explicitly is so gaps are locatable and fixable rather than hidden inside a vague, unauditable system prompt.',
        'The href for this page (`/build-verification`) is a stable, canonical identifier once the pillar crosses the platform\'s own indexability bar -- so it\'s reasonable to bookmark or cite directly when tracking an evaluation decision back to the specific architectural claim that informed it.',
      ],
    },
  ],
  faq: [
    {
      question: 'What problem does CodingAgent Build Verification actually solve?',
      answer: 'Automated build pipeline invocation inside clean task sandboxes to verify package resolution and compilation success. Guarantees that syntax and dependency modifications produce deployable binaries before a task is marked complete.',
    },
    {
      question: 'How is completion or correctness verified for this pillar?',
      answer: 'Hermetic build artifact generation with zero compilation warnings.',
    },
    {
    ...
    },
    {
      ...
      answer: 'It is categorized under Verification & Memory, tagged build, compiler, hermetic.',
    },
    {
      question: 'Who should read this page before adopting CodingAgent Build Verification?',
    ...
    {
      question: 'What\'s the recommended rollout sequence for CodingAgent Build Verification?',
    ...
    {
      question: 'How does CodingAgent Build Verification fail -- does it fail open or fail closed?',
