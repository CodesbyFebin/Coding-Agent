    ...
    faq: [
        {
            ...
            answer: 'An open, JSON-RPC-based standard through which AI agents discover and invoke external tools, resources and prompts with schema-validated, policy-governed, auditable calls.'
        },
        {
            question: 'How many MCP servers can one agent use at once?',
            answer: 'As many as policy allows, each in its own namespace. The practical limit is attention and review discipline rather than the protocol: the effective capability set should stay small enough that operators can actually reason about it, and the audit ledger will show which servers earn their place.'
        },
        {
            question: 'Does MCP work for read-only introspection as well as actions?',
            answer: 'Yes. Resources and read-only tools are first-class primitives, and schema lookup, documentation search or dependency-graph queries are often the highest-value, lowest-risk first integrations for a team.'
        },
        {
            question: 'What happens when two servers expose overlapping tools?',
            answer: 'Namespace isolation keeps them distinct, and the policy layer decides which is callable in each context. Overlaps are a review signal, not a runtime conflict, because the effective capability set is computed, not assumed.'
        },
        {
            question: 'Is MCP suitable for streaming or long-running operations?',
            answer: 'Yes, with engineering care: progress notifications and chunked outputs exist for long operations, and the resilience layer, heartbeats, backoff and buffer limits, exists precisely for servers that take minutes rather than milliseconds.'
        },
        {
            question: 'How does MCP relate to agent memory and context management?',
            answer: 'Tool outputs become part of model context, which makes MCP a context-management concern as much as an integration one: size-bounded responses, summarized large results and redaction keep the capability fabric from consuming the window the reasoning actually needs.'
        },
        {
            question: 'Can one MCP server serve multiple agents with different permissions?',
            answer: 'Yes, and that is the recommended pattern: the server declares capabilities once while each agent\u2019s policy layer computes its own effective set, so a read-only CI agent and a privileged operator agent can share the same server safely.'
        },
        {
            question: 'How do I version an MCP server safely?',
            answer: 'Publish a versioned capability manifest with every release, run contract fixtures against each version, and require manifest diffs in review; agents then recompute the policy intersection on version change instead of inheriting silent capability drift.'
        },
        {
            question: 'What does a well-behaved MCP error look like?',
            answer: 'A structured, bounded message the agent can act on: which argument failed which schema constraint, and what a valid shape would be. Raw stack traces and unbounded dumps are treated as hygiene failures because they waste context and can leak internals.'
        },
        {
            question: 'Is MCP only for large organizations?',
            answer: 'No. A two-person team benefits the first time one integration, say a schema lookup server, serves both an IDE assistant and a CI reviewer. The protocol removes per-agent connector work regardless of scale.'
        },
        {
            question: 'What is the Model Context Protocol in one sentence?',
            answer: 'An open, JSON-RPC-based standard through which AI agents discover and invoke external tools, resources and prompts with schema-validated, policy-governed, auditable calls.',
        },
        {
            ...
            answer: 'Function calling is an agent-vendor format for describing callable functions to a model. MCP is a transport- and vendor-neutral protocol for discovering, negotiating, executing and auditing those capabilities across many tools and agents — with policy evaluation as a first-class step.',
        },
        {
            question: 'Which transport should a local coding agent use?',
            answer: 'stdio for local tools such as filesystem, git and build servers — it is a local subprocess with no network surface and minimal latency. Use HTTP-based transports for shared team services, and pair them with authentication and egress policy.',
        },
        {
            question: 'Can MCP tools access data the agent should not see?',
            answer: 'Only if policy allows it. The effective capability set is the intersection of server capabilities, agent mode and project policy; destructive or sensitive tools can be excluded from that set entirely, so the model never sees them as callable options.',
        },
        {
            question: 'How do I stop a misbehaving MCP server from polluting the agent?',
            answer: 'Treat server output as untrusted input: sanitize before model ingestion, bound output sizes, redact secrets, taint-track flows, and record every decision in the audit ledger so anomalies are visible in review.',
        },
    ],
    sources: [
