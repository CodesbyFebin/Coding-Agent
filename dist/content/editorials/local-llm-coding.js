    ...
    faq: [
        {
            ...
            answer: 'A 16 GB-memory machine runs capable 7B–8B code models via Ollama or llama.cpp; 24–64 GB unlocks 32B-class models for heavier refactors. Team-serving deployments use vLLM on A100/H100 or multi-GPU workstations.'
        },
        {
            question: 'Which open models are best for coding today?',
            answer: 'The current open code families in the 7B to 32B class are strong on bounded engineering work; the routing policy tracks per-model verification history on your repository, which matters more than any general leaderboard.'
        },
        {
            question: 'How do I keep context windows from truncating mid-mission?',
            answer: 'Declare window requirements during routing, use AST-aware pruning and compaction, and let health probes confirm VRAM headroom before dispatch rather than discovering limits at generation time.'
        },
        {
            question: 'Can local models run in CI?',
            answer: 'Yes. llama.cpp runs GGUF models on standard runners without GPUs, and a shared vLLM endpoint can serve CI workers the same governed models the fleet uses, with the same policy checks.'
        },
        {
            question: 'How often do local models need updating?',
            answer: 'Pin versions, evaluate each new release against your benchmark set, and promote through the same verification gates as any dependency; the audit ledger makes each promotion reproducible.'
        },
        {
            question: 'What is the single biggest operational mistake to avoid?',
            answer: 'Routing work to a model class that cannot verify it. The task matrix plus verification history exists to prevent that failure, and it is the difference between local-first that works and local-first that gets abandoned.'
        },
        {
            question: 'What hardware do I need for local-LLM coding?',
            answer: 'A 16 GB-memory machine runs capable 7B–14B code models via Ollama or llama.cpp; 24–64 GB unlocks 32B-class models for heavier refactors. Team-serving deployments use vLLM on A100/H100 or multi-GPU workstations.',
        },
        {
            question: 'Are local models good enough for real refactoring?',
            answer: 'For bounded, verifiable work — mechanical refactors, tests, migrations with clear acceptance criteria — current open code models verify well, and per-model pass-rate history in your routing policy shows it. The hardest architectural reasoning still justifies cloud frontier models under policy.',
        },
        {
            question: 'Does local inference mean no data ever leaves my machine?',
            answer: 'When routing locks to local endpoints, inference happens on your hardware. Air-gapped mode additionally verifies zero external network egress at the sandbox level, which is the pattern for defense, banking and regulated environments.',
        },
        {
            question: 'Can I mix local and cloud models in one mission?',
            answer: 'Yes — the router dispatches per work unit: local models for code touching confidential files, cloud models for shareable reasoning-heavy units, with every routing decision recorded in the audit ledger.',
        },
    ],
    sources: [
