    ...
    faq: [
        {
            ...
            answer: 'A software system that plans and executes engineering tasks using a language model, repository context, tools, permissions and tests, completing work only when independent verification confirms it.',
        },
        {
            question: 'How is an agent different from GitHub Copilot-style autocomplete?',
            answer: 'Autocomplete suggests tokens inside the file you are editing and leaves execution to you. An agent decomposes multi-step goals, edits across files, runs tools, and verifies its own output with builds and tests before claiming completion.',
        },
        {
            question: 'Can an AI coding agent push directly to production?',
            answer: 'Not in a governed architecture. Deployments are classified ASK or DENY in the default policy posture: the agent prepares the change, evidence is attached, and a human operator signs the approval that is recorded in the audit ledger.',
        },
        {
            question: 'Do coding agents need cloud models?',
            answer: 'No. Local runtimes such as Ollama, vLLM and llama.cpp are first-class targets in CodingAgent\u2019s routing fabric, and confidential repositories are routed to local inference by policy so proprietary code never leaves your hardware.',
        },
        {
            question: 'How do you stop an agent from hallucinating APIs?',
            answer: 'Hallucination defense is structural: AST validity checks, symbol resolution against real repository definitions, typecheck gates and test execution. A fabricated API fails compilation long before a human reviews it.',
        },
        {
            question: 'What is the fastest safe way to start?',
            answer: 'Run Plan and Ask modes read-only for two weeks, review the task graphs, then enable writes into ephemeral worktrees with hermetic verification — keeping deploy, database and network capabilities behind approval gates.',
        },
    ],
    sources: [
        {
